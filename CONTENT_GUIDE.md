...
`critical` — тема ниже 50% освоенности ограничивает готовность потолком 69%.

Критическая тема обязана быть обязательной — это проверяется. И обязана иметь
и вопросы, и хотя бы один кейс: потолок, который нечем снять практикой,
превращается в тупик. Списки в двух местах (`critical_topic_ids` трека и
`critical: true` у темы) обязаны совпадать; расхождение валидатор считает
ошибкой.

`volatility` (необязательно): `low` | `medium` | `high`, по умолчанию `low`.
...
**на записи** в поле `content_reviewed_at` (формат `ГГГГ-ММ-ДД`).

| Уровень | Что относится | Норма сверки |
|---|---|---|
| `high` | GA4, интерфейс и статусы Search Console, поведение инструментов, данные вакансии | 90 дней |
| `medium` | поведение поиска Google, Core Web Vitals, JS SEO | 180 дней |
| `low` | HTTP, HTML, базовые понятия | по необходимости |

Правила:

* запись, относящаяся к теме с `volatility: "high"`, **обязана** нести
  `content_reviewed_at` — иначе непонятно, когда её последний раз сверяли с
  документацией;
* дата в будущем — ошибка: сверка не могла состояться;
* просроченная сверка — предупреждение и строка в отчёте о контенте, а не
  падение CI: краснеть от того, что прошло время, проверка не должна;
* у вакансии метка стоит на самой записи (`volatility` + `content_reviewed_at`):
  объявление правит работодатель, а не мы.

**Аннотировать всё подряд нельзя.** Поле, проставленное «на всякий случай»,
...
`how_to_check` — что человек делает **руками**, чтобы увидеть это на живом
сайте: какой отчёт открыть, что с чем сравнить, где смотреть. Не пересказ
теории и не «изучите документацию».

`tools` — чем именно проверять. Поля заполняются **парой**: проверка без
инструмента и инструмент без проверки одинаково бесполезны, и валидатор
считает половину заполненной пары ошибкой.
...
**Режимы сессии** объявлены в `session_modes` рядом с данными: состав блоков —
это контент, а не разметка экрана. Идентификатор `full` менять нельзя: по нему
считаются полные сессии в готовности и в достижениях. Если у трека нет вопросов
какой-то категории режима, блок просто пропускается, а валидатор пишет
предупреждение.

**Необязательные блоки вопроса:**

* `follow_ups` — уточняющие вопросы, которые задаст интервьюер по вашему же
  ответу: `[{ "question": "…", "expect": "чего ждут" }]`. Настоящее
  собеседование идёт вглубь, а не списком;
* `answer_depths` — тот же ответ на `30`, `60` и `120` секунд. Все три уровня
  обязательны вместе, и объём обязан возрастать: валидатор ловит перепутанные
  местами глубины;
* `explain_to` — адресат для задания «объясни простыми словами»
  (например, «проектному менеджеру, который не слышал про SEO»). Меняет
  требования к ответу, поэтому показывается до того, как человек начнёт
  отвечать.
