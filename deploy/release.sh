...
# Каталоги со стоянием сервиса не трогаем ни при каких обстоятельствах.
ssh "$HOST" APP_DIR="$APP_DIR" bash -s <"$MANIFEST" <<'REMOTE'
set -euo pipefail
cd "$APP_DIR"
sort >/tmp/itb-manifest.$$
find . -type f \
  ! -path './.venv/*' \
  ! -path './progress/*' \
  ! -path './api/backups/*' \
  ...
  ! -name '*.pyc' \
  -printf '%P\n' | sort | comm -23 - /tmp/itb-manifest.$$ |
while IFS= read -r f; do
  echo "  удаляю лишний: $f"
  rm -f -- "$f"
done
rm -f /tmp/itb-manifest.$$
REMOTE
